...
let
  install_talos = pkgs.writeShellScriptBin "install_talos" ''
    ...
    # Set TALOS_DIR - can be overridden by environment variable
    ...
    INITIAL_NODE=$(prompt_with_default "Enter the initial node IP for configuration" "192.168.8.161")
    ...
    talosctl apply-config --insecure --mode=interactive --nodes "$INITIAL_NODE"
    ...
    export CONTROL_PLANE_IP=$(prompt_with_default "Enter Control Plane IP" "192.168.8.161")
    export CLUSTER_NAME=$(prompt_with_default "Enter Cluster Name" "pissiCluster")
    ...
    if [ ''${#WORKER_IPS[@]} -eq 0 ]; then
        # Set default worker IPs if none provided
        log_info "Using default worker IPs: 192.168.8.162, 192.168.8.163"
        WORKER_IPS=("192.168.8.162" "192.168.8.163")
    fi
    ...
    echo ""
    export DISK_NAME=$(prompt_with_default "Enter disk name to use for installation (e.g., nvme, sda)" "nvme")
    ...
    echo ""
    log_info "Waiting for cluster to initialize (this may take a few minutes)..."
    sleep 10
  ...
  check_talos = pkgs.writeShellScriptBin "check_talos" ''
    ...
    CONTROL_PLANE_IP=''${1:-192.168.8.161}
...
pkgs.mkShell {
  ...
  shellHook = ''
    ...
    echo ""
    echo "Configuration location: \$HOME/.talos (or set TALOS_DIR)"
    echo "To use a different location: export TALOS_DIR=/path/to/talos"
    echo ""
  '';
}